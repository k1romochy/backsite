...
from user.crud import get_user_by_username, get_user_by_id
load_dotenv()
...
async def get_current_user(
    ...
    if session_id:
        session_obj = await session.execute(
            select(SessionModel).where(SessionModel.session_id == session_id).options(joinedload(SessionModel.user))
        )
